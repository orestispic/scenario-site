...
Les mots owner/editor/viewer sont à ce stade des identifiants de fixture, pas des privilèges tirés des métadonnées Auth. Les rôles Studio devront être attribués uniquement par les RPC serveur après création de la fixture commerciale autorisée. Stripe et l'IA restent fermés par des adaptateurs indisponibles sans accès réseau tant que leur configuration de test complète n'est pas fournie. L'API non payante peut ainsi démarrer sans clé factice ; l'environnement `production` conserve l'obligation stricte de configurer les deux fournisseurs. Restent bloqués avant une bêta fonctionnelle : attribution de droits synthétiques et parcours Studio hébergé, Stripe exclusivement test, IA explicitement autorisée et réconciliation intégrée du canal avec le journal Supabase append-only. Aucun paiement ou appel IA réel n'a été déclenché.
