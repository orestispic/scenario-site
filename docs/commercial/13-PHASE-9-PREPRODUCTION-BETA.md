...
Un essai interactif prolongé a ensuite révélé une saturation `collaboration_capacity_reached` : le client interrogeait la route de rattrapage toutes les 500 ms, au-delà de sa fenêtre distribuée de 60 requêtes par minute, puis recréait un canal après chaque refus transitoire. Le client espace désormais les polls inactifs de deux secondes et vérifie d'abord la connexion existante par heartbeat avant de demander un nouveau ticket. Le contrôle hébergé clôt aussi ses sessions via le Worker afin de révoquer toutes ses présences, y compris lorsqu'une exécution précédente s'arrête avant d'avoir mémorisé les identifiants de connexion. Après nettoyage, le parcours hébergé complet repasse avec les trois rôles, 22 événements monotones, une opération append-only unique, le snapshot privé existant et une reprise owner au curseur `22`.
...
Cette tranche est validée localement par 96 tests, dont le merge du document complet, l'ordre upload/RPC, la survie de l'artefact à un remplacement d'isolate, l'idempotence et les contrôles SQL statiques. Le build web, le typecheck, le lint ciblé, le format, la recherche de secrets et la compilation Worker à blanc passent. Le moteur Docker local n'étant plus disponible au moment de ce contrôle, la nouvelle migration n'est pas présentée comme exécutée sur PostgreSQL local. Avant l'autorisation externe suivante, elle n'était pas encore appliquée au projet hébergé et le nouveau Worker n'était pas encore déployé.

Après cette autorisation, `20260919000000_studio_snapshot_reconciliation.sql` a été appliquée seule au projet Supabase de préproduction. Les douze versions locales et distantes correspondent et `supabase db lint --linked --level error` ne remonte aucun résultat. Le commit `2a3e442` a ensuite été déployé sur la version Cloudflare `441f175d-e0ee-4af1-a27e-92f6cccc3059`.

Le parcours hébergé post-déploiement passe avec les trois comptes synthétiques : trois présences et rôles observés, ticket réutilisé refusé, opération existante rejouée deux fois sans doublon, écriture viewer refusée, trois événements rattrapés dans l'ordre, une seule opération et son accusé dans Supabase, snapshot privé créé, checksum de l'objet relu identique, version cloud append-only correspondante et reprise owner au curseur de canal `3`. Les sessions et connexions du contrôle ont été fermées en sortie. Aucun paiement, appel IA ou notification n'a été déclenché.
...
Un environnement Stripe isolé a été créé manuellement. Les produits Auteur IA et Studio disposent chacun d'un tarif mensuel et annuel conforme au catalogue serveur. Leurs quatre identifiants `price_…` ne sont pas committés : `phase9:stripe:bind` les reçoit en arguments, vérifie leur format et leur unicité, revalide offre, période, devise et montant contre le catalogue Supabase actif, puis crée ou rejoue les quatre liaisons `prices` avec des UUID déterministes. Toute liaison existante différente échoue sans écrasement.

La clé `sk_test_…` et le secret de signature `whsec_…` ont été saisis directement comme secrets chiffrés du Worker, jamais transmis au dépôt. L'ajout du second secret a produit la version Cloudflare `36cbf2fb-81bd-4f56-8b67-35e71744cbcd`; `/v1/config` répond ensuite HTTP 200 avec l'origine locale exacte et un `request_id`. Le webhook test écoute uniquement Checkout terminé, création/mise à jour/suppression d'abonnement, facture payée et facture échouée. Aucun événement réel n'a encore été envoyé.

Stripe Basil et Clover placent les périodes courantes sur `items.data[]`, contrairement aux anciennes versions qui les plaçaient aussi sur l'abonnement. La migration append-only `20260920000000_stripe_current_api_periods.sql` conserve l'événement signé brut et son empreinte, appelle les protections phase 4 existantes, puis complète atomiquement la projection et le snapshot de droits depuis les périodes du premier item lorsque les champs historiques sont absents. Elle conserve la signature RPC publique et n'accorde l'exécution qu'au `service_role`.

Après autorisation explicite, cette migration a été appliquée seule au projet de préproduction. Les treize migrations locales et distantes correspondent et le lint PostgreSQL distant ne remonte aucune erreur. `phase9:stripe:bind` a créé les quatre liaisons puis les a rejouées sans doublon (`4 created`, puis `4 replayed`). `phase9:stripe:validate` a ouvert une session owner synthétique, vérifié via `/v2/billing` quatre sélections serveur distinctes avec offre, période, EUR, montant, `testMode` et `request_id`, puis fermé immédiatement la session. Aucun Checkout n'a encore été créé et aucun webhook Stripe n'a encore été reçu.

Stripe test, sa compatibilité de webhook et ses quatre liaisons serveur sont configurés ; le prochain contrôle est un Checkout avec carte de test puis la réception et le replay de ses webhooks. L'IA reste fermée par un adaptateur indisponible sans accès réseau ; l'environnement `production` conserve l'obligation stricte de configurer ses fournisseurs. La persistance intégrée des snapshots/compactions n'est plus bloquante. Restent avant une bêta fonctionnelle complète : Checkout et webhook Stripe test validés, IA explicitement autorisée et validation multi-machine du canal temps réel. Aucun paiement, notification ou appel IA réel n'a été déclenché.
