...
Stripe et l'IA restent fermés par des adaptateurs indisponibles sans accès réseau tant que leur configuration de test complète n'est pas fournie. L'API non payante peut ainsi démarrer sans clé factice ; l'environnement `production` conserve l'obligation stricte de configurer les deux fournisseurs. Restent bloqués avant une bêta fonctionnelle complète : Stripe exclusivement test, IA explicitement autorisée, validation multi-machine du canal temps réel et persistance intégrée des snapshots/compactions dans les versions cloud append-only. Aucun paiement, notification ou appel IA réel n'a été déclenché.
