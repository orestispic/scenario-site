...
Stripe et l'IA restent fermés par des adaptateurs indisponibles sans accès réseau tant que leur configuration de test complète n'est pas fournie. L'API non payante peut ainsi démarrer sans clé factice ; l'environnement `production` conserve l'obligation stricte de configurer les deux fournisseurs. La persistance intégrée des snapshots/compactions n'est plus bloquante. Restent avant une bêta fonctionnelle complète : Stripe exclusivement test, IA explicitement autorisée et validation multi-machine du canal temps réel. Aucun paiement, notification ou appel IA réel n'a été déclenché.
