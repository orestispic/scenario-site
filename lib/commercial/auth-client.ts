...
export interface AuthClient {
  ...
  getBilling(accessToken: string): Promise<BillingOverviewResponse>;
...
export function createSupabaseAuthClient(options: {
  ...
  async function supabase(path: string, body: unknown): Promise<Response> {
    ...
    if (!response.ok)
      throw new Error('Le service de compte a refusé la demande.');
    return response;
  }
  ...
  return {
    ...
    async refreshSession(refreshToken) {
      return readSession(
        await supabase('/token?grant_type=refresh_token', {
          refresh_token: refreshToken,
        }),
      );
    },
    async requestPasswordReset(email) {
      await supabase('/recover', { email });
    },
    ...
    getEntitlements: (accessToken) =>
      api<EntitlementsResponse>('/v1/entitlements', accessToken),
    getBilling: (accessToken) =>
      api<BillingOverviewResponse>('/v2/billing', accessToken),
    createCheckoutSession: (accessToken, input) =>
      api<CheckoutSessionResponse>(
        '/v2/checkout/sessions',
        accessToken,
        'POST',
        input,
      ),
    createBillingPortal: (accessToken, returnUrl) =>
      api<BillingPortalResponse>(
...
export function createLocalTestAuthClient(
  apiBaseUrl: string,
  ...
  return {
    signUp: async () => {
      throw new Error('Inscription simulée désactivée.');
    },
    signIn: async () => {
      throw new Error('Utilisez un profil local.');
    },
    refreshSession: async () => {
      throw new Error('Session locale non renouvelable.');
    },
    requestPasswordReset: async () => {
      throw new Error('Récupération simulée désactivée.');
    },
    ...
    getEntitlements: (token) =>
      api<EntitlementsResponse>('/v1/entitlements', token),
    getBilling: (token) => api<BillingOverviewResponse>('/v2/billing', token),
    createCheckoutSession: (token, input) =>
      api<CheckoutSessionResponse>(
        '/v2/checkout/sessions',
        token,
        'POST',
    ...
    createBillingPortal: (token, returnUrl) =>
      api<BillingPortalResponse>('/v2/billing/portal-sessions', token, 'POST', {
        returnUrl,
      }),
  };
}
