...
if (
  ![
    ...
    'https://scenario-site-5f9pewvbh-orepicard-4993s-projects.vercel.app',
    'https://senario.app',
    'https://www.senario.app',
  ].includes(origin)
)
...
const { chromium } = await import(
  process.env.SCENARIO_PLAYWRIGHT_PATH
...
await context.route('**/*', async (route) => {
  ...
  if (hosted) {
    if (url.origin === auth && url.pathname === '/auth/v1/token')
  ...
  if (url.origin === auth) {
    if (
      ...
      JSON.parse(request.postData() ?? '{}').type === 'signup'
    )
      confirmed++;
    ...
    const data = ['/auth/v1/token', '/auth/v1/verify'].includes(url.pathname)
      ? {
          access_token: 'local-test:studio',
          refresh_token: 'synthetic-memory-refresh',
          expires_in: 3600,
        }
      : {};
  ...
  if (url.origin === api) {
    if (
      ...
      return route.fulfill({
        json: {
          checkoutUrl: 'https://checkout.stripe.com/c/pay/cs_test_isolated',
          testMode: true,
        },
  ...
  if (url.origin === 'https://checkout.stripe.com')
    return route.fulfill({
      ...
      body: '<h1>Checkout simulé, aucun paiement</h1>',
    });
  return route.abort();
});
const page = await context.newPage();
page.on('requestfailed', (request) => {
  if (new URL(request.url()).pathname === '/v11/catalog')
    console.log('Catalogue network failure:', request.failure()?.errorText);
});
page.on('pageerror', () =>
  failures.push('Browser exception (details intentionally omitted)'),
);
...
try {
  ...
  assert.match(
    ...
    /la meilleure\s+page blanche/,
  );
  assert.equal(await page.locator('#offres').count(), 0);
  mkdirSync('outputs', { recursive: true });
  await page.screenshot({
    path: 'outputs/redesign-home-desktop.png',
    fullPage: true,
  });
  await page
    ...
    .first()
    .click();
  assert.equal(new URL(page.url()).pathname, '/offres');
  await page
    .locator('#offres .offer')
    .first()
  ...
  assert.deepEqual(await page.locator('#offres .offer h3').allTextContents(), [
    'Gratuite',
    'Auteur',
    'Studio',
  ]);
  assert.equal(
    await page
      .getByRole('switch', { name: 'Afficher les prix mensuels' })
      .first()
  ...
  assert.equal(
    await page
      .getByRole('switch', { name: 'Afficher les prix annuels' })
      .first()
      .getAttribute('aria-checked'),
    'true',
  );
  assert.equal(
    await page
      .locator('#offres .offer')
      .nth(1)
      .getByText('Facturation mensuelle')
      .count(),
    1,
  );
  await page
  ...
  if (hosted) {
    const values = environment('.env.phase9.accounts.local');
    email = values.PHASE9_OWNER_EMAIL;
    password = values.PHASE9_OWNER_PASSWORD;
  }
  await page.screenshot({
    path: 'outputs/redesign-offers-desktop.png',
    fullPage: true,
  });
  await page
  ...
  if (!hosted) {
    ...
    assert.equal(new URL(page.url()).pathname, '/reinitialisation');
    await page.getByRole('button', { name: 'Envoyer le lien' }).click();
    await page
      .getByRole('status')
      .getByText(/un lien de récupération sera envoyé/)
      .waitFor();
  ...
  await page
    ...
    .waitFor();
  assert.equal(new URL(page.url()).pathname, '/compte');
  await page
    .getByRole('link', { name: 'Voir les offres', exact: true })
    .first()
    .click();
  await page
    .getByRole('button', { name: 'Essayer Auteur en mode test' })
    .waitFor();
  await page.goBack();
  await page
    .getByRole('button', { name: 'Se déconnecter', exact: true })
  ...
  mkdirSync('outputs', { recursive: true });
  await page.screenshot({
    path: `outputs/phase11-site-${hosted ? 'hosted' : 'local'}-mobile.png`,
    fullPage: true,
  });
  ...
  if (!hosted) {
    ...
    await page
      ...
      .click();
    await page
      .getByRole('heading', { name: 'Checkout simulé, aucun paiement' })
      .waitFor({ timeout: 10_000 })
      .catch(async () => {
        console.log(
          'Checkout fixture attempts:',
          checkout,
          'UI status:',
          await page.locator('#compte output').allTextContents(),
        );
    ...
    assert.equal(page.url(), `${origin}/reinitialisation`);
    await page
      .getByRole('button', { name: 'Modifier mon mot de passe' })
      .click();
    await page
      .getByRole('status')
      .getByText(/ne sont pas identiques/)
      .waitFor();
    ...
    await page
      .getByRole('status')
      .getByText(/Mot de passe modifié/)
      .waitFor();
    assert.equal(changed, 1);
    await page.goto(`${origin}/#token_hash=${'b'.repeat(64)}&type=signup`);
    await page
      .getByRole('button', { name: 'Confirmer mon adresse', exact: true })
      .waitFor();
    assert.equal(page.url(), `${origin}/connexion`);
  ...
  for (const path of [
    ...
    await page.screenshot({
      path: `outputs/redesign-${path.slice(1) || 'home'}-mobile.png`,
      fullPage: true,
    });
  }
  assert.deepEqual(failures, []);
  console.log(
    `PASS ${hosted ? 'HOSTED Supabase/Cloudflare (read-only)' : 'LOCAL simulated'}: catalogue, account, logout, mobile, no browser token storage${hosted ? '' : ', recovery, reset, Checkout redirect'}.`,
  );
} finally {
  await context.close();
  await browser.close();
