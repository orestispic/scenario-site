...
type JwtPayload = { sub?: string; aud?: string | string[]; iss?: string; exp?: number; nbf?: number };
...
export class SupabaseJwksTokenVerifier implements TokenVerifier {
  ...
  async verify(authorizationHeader: string | null): Promise<AuthenticatedIdentity> {
    ...
    const jwk = (await this.getJwks()).keys?.find((candidate) => (candidate as JsonWebKey & { kid?: string }).kid === header.kid);
    ...
    if (!verified) throw new AuthenticationError("Signature de jeton invalide.");

    const nowSeconds = Math.floor(this.now() / 1000);
    const issuer = `${this.supabaseUrl.replace(/\/$/, "")}/auth/v1`;
    if (!payload.sub || payload.iss !== issuer || !audienceMatches(payload.aud, this.audience)) {
      throw new AuthenticationError("Claims de jeton invalides.");
    }
    if (!payload.exp || payload.exp <= nowSeconds || (payload.nbf !== undefined && payload.nbf > nowSeconds)) {
  ...
  private async getJwks(): Promise<Jwks> {
    ...
    const response = await this.fetcher(`${this.supabaseUrl.replace(/\/$/, "")}/auth/v1/.well-known/jwks.json`, {
      headers: { Accept: "application/json" },
    });
    if (!response.ok) throw new AuthenticationError("Clés de session indisponibles.");
