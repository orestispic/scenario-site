...
function localState(profile: LocalTestProfile, deviceLimit: number, entitlements: EntitlementSnapshot["entitlements"]): LocalProfileState {
  const id = LOCAL_PROFILE_IDS[profile];
  return {
    profile: {
      id,
      authUserId: `local-${profile}`,
      account: { id, email: `${profile}@example.invalid`, displayName: `Test ${profile}` },
      role: "customer",
    },
    offerCode: profile,
    deviceLimit,
...
export class LocalTestRepository implements CommercialRepository {
  private readonly profiles = new Map<string, LocalProfileState>([
    ["local-discovery", localState("discovery", 1, [{ code: "local.edit", enabled: true, value: null }])],
    ["local-author", localState("author", 2, [{ code: "local.edit", enabled: true, value: null }, { code: "ai.actions", enabled: true, value: null }])],
    ["local-studio", localState("studio", 3, [{ code: "local.edit", enabled: true, value: null }, { code: "ai.actions", enabled: true, value: null }, { code: "cloud.sync", enabled: true, value: null }])],
  ]);
  ...
  async getConfiguration() {
    return {
      ...
      offers: [...this.profiles.values()].map(({ offerCode }) => ({ code: offerCode })),
      compatibility: [],
    };
  }
  ...
  async activateDevice(profileId: string, input: ActivateDeviceInput): Promise<DeviceView> {
    ...
    const activeCount = state.devices.filter((device) => device.status === "active").length;
    if (!existing && activeCount >= state.deviceLimit) {
      throw new CommercialRepositoryError(409, "device_limit_reached", "Limite d’appareils atteinte.");
    }
    const device: DeviceView = existing ?? {
      id: input.fingerprintHash.slice(0, 8) + "-0000-4000-8000-" + input.fingerprintHash.slice(8, 20),
      label: input.label,
      platform: input.platform,
      status: "active",
      lastSeenAt: new Date().toISOString(),
    };
    Object.assign(device, { label: input.label, platform: input.platform, status: "active", lastSeenAt: new Date().toISOString() });
    if (!existing) {
  ...
  async deactivateDevice(profileId: string, deviceId: string): Promise<void> {
    const device = this.requireState(profileId).devices.find((candidate) => candidate.id === deviceId);
    if (!device) throw new CommercialRepositoryError(404, "device_not_found", "Appareil introuvable.");
  ...
  async appendAudit(event: { profileId: string | null; action: string }): Promise<void> {
    this.audit.push(event);
  }

  private findByProfileId(profileId: string): LocalProfileState | undefined {
    return [...this.profiles.values()].find((state) => state.profile.id === profileId);
  }

  private requireState(profileId: string): LocalProfileState {
    const state = this.findByProfileId(profileId);
    if (!state) throw new CommercialRepositoryError(404, "profile_missing", "Profil local introuvable.");
    return state;
  }
}
...
export class LocalTestTokenVerifier implements TokenVerifier {
  async verify(authorizationHeader: string | null): Promise<AuthenticatedIdentity> {
    const match = /^Bearer local-test:(discovery|author|studio)$/.exec(authorizationHeader ?? "");
    if (!match) throw new AuthenticationError();
    return { authUserId: `local-${match[1]}`, accessToken: authorizationHeader!.slice(7) };
  }
}
