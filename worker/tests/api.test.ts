...
import { LocalTestRepository, LocalTestTokenVerifier } from "../src/localTestRepository.ts";
...
describe("session Supabase", () => {
  it("valide signature JWKS, issuer, audience et expiration", async () => {
    ...
    const header = encodeBase64Url(new TextEncoder().encode(JSON.stringify({ alg: "ES256", kid: publicJwk.kid })));
    ...
    await assert.rejects(() => verifier.verify(`Bearer ${header}.${payload}.${encodeBase64Url(tamperedSignature)}`));
  });
});
...
describe("API commerciale v1", () => {
  ...
  function request(path: string, profile?: "discovery" | "author" | "studio", init: RequestInit = {}) {
    const headers = new Headers(init.headers);
    if (profile) headers.set("Authorization", `Bearer local-test:${profile}`);
    if (init.body) headers.set("Content-Type", "application/json");
    return worker.fetch(new Request(`https://api.example.invalid${path}`, { ...init, headers }));
  }

  it("expose uniquement la configuration publique sans session", async () => {
    const response = await request("/v1/config");
    assert.equal(response.status, 200);
    const body = await response.json() as { version: string; offlineGrantPublicKey: JsonWebKey };
  ...
  it("charge le rôle depuis le dépôt et non depuis le jeton", async () => {
    ...
    assert.equal(response.status, 200);
    const body = await response.json() as { role: string; account: { email: string } };
  ...
  it("signe un cache d’entitlements vérifiable et expirant", async () => {
    ...
    const config = await configResponse.json() as { offlineGrantPublicKey: JsonWebKey };
    const response = await request("/v1/entitlements", "studio");
    assert.equal(response.status, 200);
    const body = await response.json() as { offlineGrant: SignedOfflineGrant };
    const key = await crypto.subtle.importKey("jwk", config.offlineGrantPublicKey, { name: "ECDSA", namedCurve: "P-256" }, false, ["verify"]);
    ...
    const payload = JSON.parse(new TextDecoder().decode(decodeBase64Url(body.offlineGrant.payload))) as { expiresAt: string };
  ...
  it("applique la limite d’appareils côté serveur", async () => {
    const first = await request("/v1/devices/activate", "discovery", {
      method: "POST",
      body: JSON.stringify({ fingerprint: "first-device-fingerprint", label: "Portable", platform: "windows" }),
    });
    assert.equal(first.status, 201);
    const second = await request("/v1/devices/activate", "discovery", {
      method: "POST",
      body: JSON.stringify({ fingerprint: "second-device-fingerprint", label: "Bureau", platform: "windows" }),
    });
    assert.equal(second.status, 409);
    assert.equal((await second.json() as { code: string }).code, "device_limit_reached");
  });

  it("refuse qu’un client Auteur s’accorde Studio", async () => {
    const manipulated = await request("/v1/devices/activate", "author", {
      method: "POST",
      body: JSON.stringify({ fingerprint: "author-device-fingerprint", label: "Portable", platform: "macos", offer: "studio" }),
    });
    assert.equal(manipulated.status, 400);
    assert.equal((await manipulated.json() as { code: string }).code, "unexpected_field");

    const entitlements = await (await request("/v1/entitlements", "author")).json() as { snapshot: { entitlements: Array<{ code: string }> } };
    assert.equal(entitlements.snapshot.entitlements.some(({ code }) => code === "cloud.sync"), false);
  });
  ...
  it("couvre appareils, usage et déconnexion", async () => {
    ...
    const logout = await request("/v1/auth/logout", "studio", { method: "POST", body: "{}" });
    assert.equal(logout.status, 204);
    assert.ok(repository.audit.some(({ action }) => action === "session.logout"));
  });
  ...
  it("refuse les requêtes au-delà de la fenêtre de rate limit", async () => {
    ...
    const first = await limitedWorker.fetch(new Request("https://api.example.invalid/v1/me", { headers: { Authorization: "Bearer local-test:studio" } }));
    const second = await limitedWorker.fetch(new Request("https://api.example.invalid/v1/me", { headers: { Authorization: "Bearer local-test:studio" } }));
